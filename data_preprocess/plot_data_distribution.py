...
def plot_trace_distribution(trace, output_path=None):
    ...
    pgv_threshold = np.log10([0.007, 0.019, 0.057, 0.15, 0.3, 0.5, 0.8, 1.4, 3])
    fig, ax = plt.subplots(figsize=(7, 7))
    sns.histplot(trace, x="pgv", hue="from", alpha=1, ax=ax, bins=32)
    for i in range(len(pgv_threshold) - 1):
        ax.text((pgv_threshold[i] + pga_threshold[i + 1]) / 2, 10000, label[i])
    ax.vlines(pgv_threshold, 0, 40000, linestyles="dotted", color="k")
